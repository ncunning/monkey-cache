...
[assembly: InternalsVisibleTo("MonkeyCache.TestsRealm")]
...
namespace MonkeyCache
{
	...
	public static class BarrelUtils
	{
		...
		/// <param name="timeSpan"></param>
		/// <returns></returns>
		internal static DateTimeOffset GetExpirationOffset(TimeSpan timeSpan)
		{
			try
			{
				return DateTimeOffset.UtcNow.Add(timeSpan);
			}
			catch
			{
				if (timeSpan.Milliseconds < 0)
